...
import { useAuthStore } from "@/store"; // Updated to use the correct store
...
export default function SignInPage() {
  ...
  const { login, error, clearError } = useAuthStore(); // Get login function and error state from auth store
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Instead of immediately checking for errors, we can add a more explicit check
      // that allows us to handle specific error conditions
      ...
      if (currentState.error) {
        ...
        // If this is an email verification issue, we can take specific action
        if (currentState.verificationEmail) {
          // Maybe offer to resend verification email
          // Or show a special message
        }
